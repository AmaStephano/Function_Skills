...
def repeat_string(input_string, num_of_times):
...
def total_meal_price(meal_price, tip_percentage=False):

    if not tip_percentage:
        return meal_price + (meal_price * .15)
    else:
...
output_list = sign_and_parity(5)
sign = output_list[0]
parity = output_list[1]
print sign
print parity
...
def write_letter(recipient_name, job, sender):
    
    greeting = full_title(recipient_name, job)

    print "Dear %s, I think you are amazing! Sincerely, %s" % (greeting, sender)
